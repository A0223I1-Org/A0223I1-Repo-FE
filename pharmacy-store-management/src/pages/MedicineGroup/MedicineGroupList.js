...
import * as medicineGroupService
    from "../../utils/InformationService/MedicineGroupManagementService/MedicineGroupService"
import {toast} from "react-toastify";
import Header from "../../components/header/Header";
import Nav from "../../components/nav/Nav";
import styled from "styled-components";

const MedicineGroupListCSS = styled.div`
  body {
    padding: 20px
  }
                    ...
                    <div className="main-right">
                        <div className="container">
                                ...
                                <table className="table table-bordered">
                                    ...
                                    <tbody>
                                    {
                                        medicineGroup?.map((item) => (
                                            <tr key={item.medicineGroupId} onClick={() => selectRow(item)}
                                                className={medicineGroupId === item.medicineGroupId ? 'table-primary' : ''}>
                            ...
                            <fieldset className="border border-dark border-3 rounded-3 p-3 mt-4">
                                ...
                                <form>
                                    <div className="mb-3">
                                        ...
                                        <input type="text" className="form-control" id="groupId" value={medicineGroupId}
                                               onChange={(e) => setMedicineGroupId(e.target.value)} disabled/>
                                    </div>
                                    <div className="mb-3">
                                        <label htmlFor="groupName" className="form-label">Tên nhóm thuốc</label>
                                        <input type="text" className="form-control" id="groupName"
                                               value={medicineGroupName}
                                               onChange={(e) => setMedicineGroupName(e.target.value)}/>
                            ...
                            <div className="my-3" style={{textAlign: 'right'}}>
                                <button className="btn btn-success" id="addBtn" disabled={disableButton}
                                        onClick={() => createNewMedicineGroup()}>
                                    <i className="bi bi-plus-circle"></i> {isAdding ? "Hoàn Thành" : "Thêm"}</button>
                                <button className="btn btn-custom" id="editBtn" disabled={disableButton}
                                        onClick={() => updateMedicineGroupById()}><i
                                    className="bi bi-pencil-square"></i> Sửa
                                </button>
                                <button className="btn btn-danger" id="deleteBtn" onClick={() => confirmDelete()}><i
                                    className="bi bi-x-circle"></i> Xóa
                                </button>
                                <button className="btn btn-primary" id="backBtn" onClick={() => backButton()}><i
                            ...
                            <div className={`modal ${showDeleteModal ? 'show' : ''}`}
                                 ...
                                 id="deleteModal" tabIndex="-1" aria-labelledby="deleteModalLabel"
                                    ...
                                    <div className="modal-content">
                                        ...
                                        <div className="modal-footer">
                                            ...
                                            <button type="button" className="btn btn-danger"
                                                    onClick={() => deleteMedicineGroupById()}>Delete
                                            </button>
                                        </div>
                                    </div>
                                </div>
